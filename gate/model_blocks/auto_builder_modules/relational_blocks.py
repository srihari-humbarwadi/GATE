...
def image_to_patch_coordinates(
    x: torch.Tensor, patch_size: int, stride: int, dilation: int
):
    patches = []
    ...
    for i in range(0, x.shape[2] - (patch_size - 1), stride):
        for j in range(0, x.shape[3] - (patch_size - 1), stride):
            if i + x_range <= x.shape[2] and j + y_range <= x.shape[3]:
                coord = torch.Tensor(
                    [
                        x * x.shape[2] + y
...
def image_to_vectors(x: torch.Tensor, indexes_of_patches: torch.Tensor):
    ...
    out = torch.index_select(out, dim=2, index=indexes_of_patches.to(x.device))
