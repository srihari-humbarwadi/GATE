...
from gate.configs.datamodule.few_shot_classification import (
    FewShotDataModuleConfig, FewShotTransformConfig, VGGFlowersFewShotDatasetConfig)
from gate.configs.datasets.transforms import (vgg_flowers_query_set_transforms,
                                              vgg_flowers_support_set_transforms)
...
def test_aircraft_fewshot_datamodules(
    ...
    dataset_config = VGGFlowersFewShotDatasetConfig(
        dataset_root=".test/datasets/vgg_flowers",
        split_name="train",
        download=True,
        num_episodes=100,
    ...
    transform_train = FewShotTransformConfig(
        support_set_input_transform=vgg_flowers_support_set_transforms(),
        query_set_input_transform=vgg_flowers_query_set_transforms(),
