...
from gate.configs.datamodule.few_shot_classification import (CUB200FewShotDatasetConfig,
                                                             FewShotDataModuleConfig,
                                                             FewShotTransformConfig)
from gate.configs.datasets.transforms import (cub200_query_set_transforms,
                                              cub200_support_set_transforms)
...
@pytest.mark.parametrize("variable_num_samples_per_class", [True, True])
@pytest.mark.parametrize("variable_num_classes_per_set", [True, True])
def test_cub200_fewshot_datamodules(
    datamodule_config_class,
    batch_size,
    num_workers,
    pin_memory,
    ...
    dataset_config = CUB200FewShotDatasetConfig(
        dataset_root=".test/datasets/cub200",
        split_name="train",
        download=True,
        num_episodes=100,
    ...
    transform_train = FewShotTransformConfig(
        support_set_input_transform=cub200_support_set_transforms(),
        query_set_input_transform=cub200_query_set_transforms(),
