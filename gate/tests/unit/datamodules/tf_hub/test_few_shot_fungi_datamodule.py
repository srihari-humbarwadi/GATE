...
from gate.configs.datasets.transforms import (fungi_query_set_transforms,
...
@pytest.mark.parametrize("variable_num_classes_per_set", [True])
def test_fewshot_datamodules(
    ...
    dataset_config = FungiFewShotDatasetConfig(
        dataset_root=".test/datasets/fungi",
        split_name="train",
        download=True,
        num_episodes=100,
    ...
    transform_train = FewShotTransformConfig(
        support_set_input_transform=fungi_support_set_transforms(),
        query_set_input_transform=fungi_query_set_transforms(),
