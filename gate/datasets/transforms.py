...
class SuperClassExistingLabels(torch.nn.Module):
    ...
    def _group_targets(self, targets, num_classes_to_group):
        ...
        class_mappings = {
            v_item: k for k, v in enumerate(class_groupings) for v_item in v.tolist()
        }
    ...
    def forward(self, x):
        ...
        if isinstance(self.num_classes_to_group, Tuple):
            num_classes_to_group = torch.randint(
                self.min_num_classes_to_group, self.max_num_classes_to_group, (1,)
            ).item()
        else:
...
class RandomCropResizeCustom(torch.nn.Module):
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(size={self.size}," f" padding={self.padding})"
        )
...
class MultipleRandomCropResizeCustom(RandomCropResizeCustom):
    ...
    def forward(self, img):
        ...
        for _ in range(num_augmentations):
            ...
            multiple_img["crop_coordinates"].append(single_img["crop_coordinates"])
