...
class FewShotClassificationDataset(Dataset):
    def __init__(
        self,
        dataset_name: str,
        dataset_root: str,
        ...
        super(FewShotClassificationDataset, self).__init__()
        ...
        # log.info(
        #     f" Dict tree of config: "
        #     f"{list(self.class_to_address_dict.keys())}"
        # )
        ...
        if self.split_config is None:
            ...
            if self.print_info:
                log.info(OmegaConf.to_yaml(self.split_config))
            self.current_class_to_address_dict = {
    ...
    def __getitem__(self, index):
        ...
        for class_name in select_classes_for_set:
            ...
            selected_samples_addresses_idx = rng.choice(
                range(
                    max(2, self.min_num_samples_per_class),
            ...
            data_inputs = [
                torch.tensor(data_inputs[i]).permute([2, 0, 1]) for i in shuffled_idx
            ]

            data_labels = [torch.tensor(data_labels[i]) for i in shuffled_idx]
        ...
        support_set_labels = (
            torch.stack(support_set_labels, dim=0)
        ...
        query_set_labels = (
            torch.stack(query_set_labels, dim=0)
